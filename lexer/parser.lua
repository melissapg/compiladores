...
function walk()
    ...
    prox = lexer.get_next_token()
    while prox.tag == 'SPACE' or prox.tag == 'COMMENT' or prox.tag == 'NEWLINE' do
        prox = walk()
    end
end
...
function parser.parseProg()
    ...
    return {tag = 'Prog', block = {tag = "Bloco", block = block}}
end
...
function parseCmd()
    ...
    if peek("do") then
        ...
        local block = parseBlock()
        eat("end")
        return {tag = "CmdDo", block = {tag = "Bloco", block = block}}
    ...
    elseif peek("while") then
        ...
        local block = parseBlock()
        eat("end")
        return {tag = "CmdWhile", exp = exp, block = {tag = "Bloco", block = block}}
    ...
    elseif peek("if") then
        ...
        return {tag = "CmdIfElse", exp = exp, block = {tag = "Bloco", block = block}, elses = elses}
    ...
    elseif peek("function") then
        ...
        local block = parseBlock()
        eat("end")
        return {tag = "CmdFunction", name = name, params = params, block = {tag = "Bloco", block = block}}
    ...
    elseif peek("NOME") then
        ...
        eat("=")
        local exp = parseExp()
        return {tag = "CmdAtribui", name = name, exp = exp}
    end

    while true do
        local exp = parseExpSufixada()
        eat("(")
        local exps = Exps()
        eat(")")
        return {tag = "CmdChamada", exp = exp, exps = exps}
    end
end
...
function parseTabela()
    ...
    while not(peek("}")) do
        if not(peek("NOME")) then
            keyvals.insert(KeyVals(int_key))
            int_key = int_key + 1
        else
            keyvals.insert(KeyVals())
        end
    end
    eat("}")
